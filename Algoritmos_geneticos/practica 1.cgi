...
@valores = (10,10,20,30,30,10,30,5);
@pesos = (0,0,3,2,5,2,7,1);
@precios = (10,20,20,10,0,10,10,0);


$alfa = 1;   ###ponderación del valor
$beta = 1;  ###ponderación del peso
$gamma = 1; ###ponderacion del coste

#cuerpo principal del código

 $num_objetos = 8; ###enunciado

#parametros de la modelización

$r = $num_objetos; #longitud cadena
$s = 2; #numero de simbolo del alfabeto
$M = 0; #M sera la suma de todos los pesos
$costemaximo = 0; #Será la suma de todos los precios


################################## pasa los pesos y los precios al intervalo [0,30] para ponderar equitativamente
for($j=0; $j<$r; $j++)
               {
               $pesos[$j] = $pesos[$j] * 30 / 7;
               }

for($j=0; $j<$r; $j++)
               {
               $precios[$j] = $precios[$j] * 30 / 20;
               }

################################### Ajuste de los presupuestos y capacidades al nuevo intervalo

$presupuesto = $presupuesto * 30 / 20;

$capacidad = $capacidad  * 30 / 7;
##################################
...
for($j=0; $j<$r; $j++)
               {
                $costemaximo = $costemaximo + $precios[$j];
               }
	...
	&mutacion;
        ...
        for($i=0; $i<$tam_pob; $i++) {
        	$aptitudes[$i] = &aptitud(@$i);
                print "individuo $i --> @$i --> peso = $peso [0,30] --> coste = $coste [0,30] --> aptitud $aptitudes[$i]\n";
        }
        &mejor;
...
sub aptitud
     ...
     $apt = 0;
     $peso = 0;
     $coste = 0;      			 ### La aptitud sera el valor
      for($j=0; $j<$r; $j++)
               {
        ...
        $apt = $apt*$alfa + $beta*($M - $peso) + $gamma*($costemaximo - $coste); #(usaremos $alfa, $beta y $gamma para ponderar)

...
@1000 = (0,2,5,$NE);   ### de un nodo a si mismo el peso es 0 para permitir repeticiones.
@1001 = ($NE,0,1,$NE); ### todos los individuos van a tener la misma longitud.
@1002 = ($NE,1,0,1);
@1003 = ($NE,$NE,$NE,0);
...
$tam_pob= 20; #NO hacerla mayor que 999
$num_generaciones = 20;
$pc = 0.5;
$pm = 0.01;
...
sub mutacion {
        ...
        for ($i=0; $i<$tam_pob; $i++){
        	for ($j=1; $j<$r-1; $j++){    ########################### PA QUE NO MUTEN NI EL PRIMERO NI EL ULTIMO
                	$aux = rand(1);
                        if($aux<=$pm) {
